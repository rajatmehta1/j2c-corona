...
public class MedicineController {
    ...
    private final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(MedicineController.class);
    private final SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");

    @RequestMapping(value = "/{city_name}/medicines",method = RequestMethod.GET)
    public String showMedicines(@PathVariable(name = "city_name", required = true) String cityName,Model model) {
        model.addAttribute("medCity","Medicines in " + cityName);
        model.addAttribute("cityName",cityName);
        return "medicines";
    }

    @RequestMapping(value = "/{city_name}/medicine/{med_name}",method = RequestMethod.GET)
...
//            logger.info("Srch Text ==> " + srchText);
       ...
       // query.setCount(20);
        ...
        List<twitter4j.Status> tweetsRst = new ArrayList<>();
        tweetsRst = result.getTweets();
        for (twitter4j.Status t : tweetsRst) {
            Tweet twt = new Tweet();
